...
export class PostService {
    ...
    async getAllPosts(offset: number, limit: number): Promise<any[]> {
        const posts = await this.postRepository
            ...
            .leftJoinAndSelect(
                'user.files',
                'userFile',
                'userFile.fileCategory = :profileCategory',
                { profileCategory: 0 },
            ) // 프로필 이미지 카테고리
            .leftJoinAndSelect(
                'post.files',
                'postFile',
                'postFile.fileCategory = :postCategory',
                { postCategory: 1 },
            ) // 포스트 파일 카테고리
            .addSelect([
    ...
    async addPost(
        ...
        if (attachFilePath) {
            newPost.fileId = await this.getNewPostFileId(
                userId,
                newPost.postId,
                attachFilePath,
            );
    ...
    async updatePost(
        ...
        // 제목, 내용, 닉네임 업데이트
        const savedPost = await this.postRepository.update(
            { postId, userId },
            { postTitle, postContent, nickname },
        );
        ...
        // 새로운 파일 추가
        const postFileId = await this.getNewPostFileId(
            userId,
            postId,
            attachFilePath,
        );
    ...
    async softDeletePost(postId: number, userId: number): Promise<any> {
        ...
        const deletePost = await this.postRepository.softDelete({
            postId,
            userId,
        });
    ...
    private async getNewPostFileId(
        userId: number,
        postId: number,
        filePath: string,
    ): Promise<any> {
        ...
        const postFile =
            await this.fileService.createPostImage(createPostImageDto);
